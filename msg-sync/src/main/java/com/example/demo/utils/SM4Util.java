...
public class SM4Util {
     ...
     * @return
     */
    public static byte[] encodeSM4(String plain, byte[] key) {
        if (plain == null || plain.equals("")) {
            return null;
        }
        for (int i = plain.getBytes().length % 16; i < 16; i++) {
            plain += "''";
        }
        return encodeSM4(plain.getBytes(), key);
    }
    ...
    public static String decodeSMS4toString(byte[] cipher, byte[] key) {
        byte[] plaintext = decodeSM4(cipher, key);
    ...
    private static byte[] encode16(byte[] plain, byte[] key) {
        ...
        SM4 sm4 = new SM4();
        sm4.sm4(plain, 16, key, cipher, ENCRYPT);
        return cipher;
    }
    ...
    private static byte[] decode16(byte[] cipher, byte[] key) {
        ...
        SM4 sm4 = new SM4();
        sm4.sm4(cipher, 16, key, plain, DECRYPT);
        return plain;
    }
    ...
    private static class SM4 {
        ...
        public int sm4(byte[] in, int inLen, byte[] key, byte[] out, int CryptFlag) {
            ...
            // int[] round_key={0};
            SM4KeyExt(key, round_key, CryptFlag);
            ...
            while (inLen >= BLOCK) {
                ...
                SM4Crypt(input, output, round_key);
